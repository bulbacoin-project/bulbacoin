...
uint256 hashGenesisBlock("0x0000068ae8f7c0bb17a6aeca55fe937d0b6ed11567057a940089f5342eada0bb"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BulbaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BulbaCoin Signed Message:\n";
...
{
    // BulbaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+30) - GetDepthInMainChain());
}
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 102600 * COIN; //1% premine in 1 block
    }
    else if(nHeight > 20 && nHeight <= 15140)
    {
       nSubsidy = 50 * COIN;
    }
    else if(nHeight > 15140 && nHeight <= 79940)
    {
       nSubsidy = 25 * COIN;
    }
    else if(nHeight > 79940 && nHeight <= 868340)
    {
       nSubsidy = 5 * COIN;
    }
...
static const int64 nTargetTimespan = 24 * 60 * 60; // 1 day
static const int64 nTargetSpacing = 40; // 1 blocks: 40 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 2160
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BulbaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, bulbacoin - DarkGravity v3, written by Evan Duffield - evan@bulbacoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
{
        static const int64 BlocksTargetSpacing = 30; // 30 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
            ...
            if (pindexLast->nHeight+1 >= 9) { DiffMode = 3; }
            else if (pindexLast->nHeight+1 >= 3) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BulbaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BulbaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000006c58c43758fdef245a08d035038f8548f03bde318a39096545611ff1138");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Cryptocurrency of Belarus";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403432007;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1865235;

        if (fTestNet)
        {
            block.nTime    = 1403373057;
            block.nNonce   = 614593;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf4a4dc8f0ca38749784249f6d33278dd8541b32c38260df7a73006c19209f7ec"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // BulbaCoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 2160 / 250)))  //2160 block a day
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BulbaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BulbaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BulbaCoinMiner(CWallet *pwallet)
{
    printf("BulbaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bulbacoin-miner");
    ...
    try { loop {
        ...
        printf("Running BulbaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BulbaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BulbaCoinMiner, pwallet));
}
